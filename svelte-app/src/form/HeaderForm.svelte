<script>
    ...
    function edit(i) {
        if(links[i].dropDown&&!links[i].content)
            links[i].content = []
        const obj = links[i]
        formStatus = i
...
<div class="border border-gray-300 m-10 px-10 pb-10">
    ...
    <div class="flex flex-row">
        <form on:submit|preventDefault={()=>newsFormSubmit()} class="w-1/3  border border-gray-300">
            ...
            <div class="m-4">
                <input class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                       type="checkbox" id="flexCheckDefault" bind:checked={$newsFormValues.dropDown}>
        ...
        <table class="ml-4 content-list w-2/3">
            <tr>
                ...
                <th class="border border-slate-300">drop down</th>
            ...
            {#each links as content, i}
                <tr>
                    ...
                    <td class="border border-slate-300 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap w-10">
                        {#if content.dropDown == true ? "true":"flase"}
                    </td>
    ...
    {#if formStatus != -1}
    {#if links[formStatus].dropDown == true}
    <div class="flex flex-row">
        <form on:submit|preventDefault={()=>newsFormSubmit()} class="w-1/3  border border-gray-300">
            <h3 class="m-4">
                {#if formStatus === -1}
        ...
        <table class="ml-4 content-list w-2/3">
            ...
            {#each links[formStatus].content as content, i}
                <tr>
                    <td class="border border-slate-300 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{content.text}</td>
                    <td class="border border-slate-300 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{content.href}</td>
                    <td class="border border-slate-300 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap w-10">
                        {#if content.dropDown == true}
                            <button on:click={()=>ddedit(i)} class="edit uppercase">edit</button>
                        {/if}
                    </td>
                    <td class="border border-slate-300 text-sm text-gray-900 font-light px-3 py-1 whitespace-nowrap">
                        <div class="flex flex-row justify-around">
